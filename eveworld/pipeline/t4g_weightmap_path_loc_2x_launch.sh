#!/usr/bin/env bash
# IGR 权重图变体 path_loc_2x —— 提交入口 (六变体并列, 无推荐; 见 t4g_weightmap_variants.md)。
...
WMAP_DIR="${WMAP_ROOT}/weightmap_cache_path_loc_2x"
OUT_ROOT="${GAGI}/eve_v2_outputs/t4g_weightmap_variants/path_loc_2x"
RUN_NAME="${RUN_NAME:-t4g_wmap_path_loc_2x_s300}"
BASE_CONFIG_MODULE="eveworld.pipeline.t4g_weightmap_path_loc_2x_config"
...
run_check() {
  ...
  "${PYTHON}" -m py_compile \
    ...
    eveworld/pipeline/t4g_weightmap_path_loc_2x_config.py
  ...
  if [[ "${N_WMAP}" -lt "${EXPECTED_SAMPLES}" ]]; then
    ...
    echo "  先生成: T4G_WMAP_ROOT=${WMAP_ROOT} ${PYTHON} eveworld/pipeline/t4g_weightmap_variants_precompute.py --variant path_loc_2x" >&2
  else
...
case "${1:-}" in
  ...
  *)
    cat <<EOF
IGR 权重图变体 path_loc_2x (六变体并列, 无推荐)
  config: ${BASE_CONFIG_MODULE}
  级别:   1x(背景) / 2x 二值
  区域:   Path/Loc. = {obj, trans, b_dest, empty} 2x (grip/distractor 不加权)
