#!/usr/bin/env bash
# IGR 权重图变体 legacy_multilevel —— 提交入口 (六变体并列, 无推荐; 见 t4g_weightmap_variants.md)。
...
WMAP_DIR="${WMAP_ROOT}/weightmap_cache_legacy_multilevel"
OUT_ROOT="${GAGI}/eve_v2_outputs/t4g_weightmap_variants/legacy_multilevel"
RUN_NAME="${RUN_NAME:-t4g_wmap_legacy_multilevel_s300}"
BASE_CONFIG_MODULE="eveworld.pipeline.t4g_weightmap_legacy_multilevel_config"
...
run_check() {
  ...
  "${PYTHON}" -m py_compile \
    ...
    eveworld/pipeline/t4g_weightmap_legacy_multilevel_config.py
  ...
  if [[ "${N_WMAP}" -lt "${EXPECTED_SAMPLES}" ]]; then
    ...
    echo "  先生成: T4G_WMAP_ROOT=${WMAP_ROOT} ${PYTHON} eveworld/pipeline/t4g_weightmap_variants_precompute.py --variant legacy_multilevel" >&2
  else
...
case "${1:-}" in
  ...
  *)
    cat <<EOF
IGR 权重图变体 legacy_multilevel (六变体并列, 无推荐)
  config: ${BASE_CONFIG_MODULE}
  级别:   0.5x(背景) / 2x / 3x / 4x / 6x 五档
  区域:   obj=4x, trans=6x, empty=2x, b_dest=3x, grip=3x, distractor=2x (与现行 t4g_weightmap 逐位一致)
