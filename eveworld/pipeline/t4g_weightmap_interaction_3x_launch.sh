#!/usr/bin/env bash
# IGR 权重图变体 interaction_3x —— 提交入口 (六变体并列, 无推荐; 见 t4g_weightmap_variants.md)。
...
WMAP_DIR="${WMAP_ROOT}/weightmap_cache_interaction_3x"
OUT_ROOT="${GAGI}/eve_v2_outputs/t4g_weightmap_variants/interaction_3x"
RUN_NAME="${RUN_NAME:-t4g_wmap_interaction_3x_s300}"
BASE_CONFIG_MODULE="eveworld.pipeline.t4g_weightmap_interaction_3x_config"
...
run_check() {
  ...
  "${PYTHON}" -m py_compile \
    ...
    eveworld/pipeline/t4g_weightmap_interaction_3x_config.py
  ...
  if [[ "${N_WMAP}" -lt "${EXPECTED_SAMPLES}" ]]; then
    ...
    echo "  先生成: T4G_WMAP_ROOT=${WMAP_ROOT} ${PYTHON} eveworld/pipeline/t4g_weightmap_variants_precompute.py --variant interaction_3x" >&2
  else
...
case "${1:-}" in
  ...
  *)
    cat <<EOF
IGR 权重图变体 interaction_3x (六变体并列, 无推荐)
  config: ${BASE_CONFIG_MODULE}
  级别:   1x(背景) / 3x 二值
  区域:   交互区 = Path/Loc.{obj,trans,b_dest,empty} + Arm/Grip.{grip} 共 3x
