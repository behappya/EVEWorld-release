...
class T5TextEncoder(torch.nn.Module):
    ...
    def encode_prompts(self, prompts: Union[str, List[str]], max_length: int = 512) -> Tuple[torch.Tensor, torch.Tensor]:
        """Encodes text prompts into hidden state representations using a T5
        encoder.
        ...
        input_ids = batch_encoding.input_ids.to(self.text_encoder.device)
        attn_mask = batch_encoding.attention_mask.to(self.text_encoder.device)
