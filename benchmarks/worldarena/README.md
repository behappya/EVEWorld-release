...
- **Appendix "MLR Evaluation Protocol"** (`app:mlr_protocol`, Algorithm 1): the deterministic GroundingDINO counting rule (initial frame + 24 uniformly sampled timestamps, gripper-overlap removal, two-consecutive-timestamp deviation rule) is implemented by `mlr_dispatch.py` + `mlr_eval.py`. The occlusion exemption of Algorithm 1 is implemented by `mlr_occlusion.py`: an over-count is always kept as deviation evidence, an under-count is exempted only when every missing instance is still occluded by the gripper (SAM2 segmentation overlap `r^occ ≥ τ_occ = 0.15` inside each missing instance's GroundingDINO box), and an exempted timestamp reverts the count to the initial inventory `N0` (which resets the two-consecutive-timestamp counter). The runnable presets — including the paper's Algorithm 1 configuration — are collected in `mlr_protocol_profiles.yaml`.
...
- **Appendix "Training Dynamics, Guidance, and Output Length"** and the RoboTwin/FlowWAM transfer arm additionally reuse the frozen MLR protocol through `mlr_cfg_flowwam_dispatch.py` and `mlr_flowwam_variants.py` (see [`../robotwin_flowwam/`](../robotwin_flowwam/)). Videos generated by the DreamGenBench main-results pipeline in [`../dreamgenbench/`](../dreamgenbench/) are scored by the same frozen runner here (`mlr_eval.py`, driven through `mlr_dispatch.py` over a DreamGenBench manifest); `eveworld/evaluation/eval_gemini_mlr_audit.py` is the independent Gemini re-check of MLR events (see [`../../eveworld/evaluation/`](../../eveworld/evaluation/)).
...
| `mlr_eval.py` | Per-shard MLR worker: GroundingDINO target counting on the condition image and 24 uniformly sampled frames per video, gripper-overlap filtering, two-consecutive-frame event rule. All protocol knobs (initial-count source, sampled-count source, sampling axis, deviation mode, occlusion rule, persistence) are exposed as flags and can also be set together via `--profile`. |
| `mlr_occlusion.py` | SAM2.1 occlusion evidence for the MLR under-count exemption: tracks the gripper and the missing instances, measures the gripper-mask overlap inside each missing instance's detection box, and applies the `r^occ ≥ τ_occ` rule (e.g. `--occlusion-rule paper_overlap`). Also runs standalone (`--video`, `--trace`, `--list-profiles`, `--compare-all`, `--self-test`). |
| `mlr_protocol_profiles.yaml` | Named MLR protocol presets — the paper's Algorithm 1 configuration, detector/rule probes, and count-source variants — selected with `--profile <name>` on `mlr_eval.py` / `mlr_dispatch.py`. Each preset is self-contained; nothing is marked as preferred. |
...
5. **Run MLR** (sharded over 8 GPUs) and re-anchor on generated first frames:
   ...
   That default is the frozen WorldArena 1.0 rule (over-count only, no occlusion evidence). To run the appendix's occlusion-aware protocol instead, select its profile and point the stack at SAM2 and GroundingDINO:

   ```bash
   SAM2_CHECKPOINT=/path/to/sam2.1_hiera_tiny.pt \
   GDINO_PATH=/path/to/grounding_dino_base \
     python benchmarks/worldarena/mlr_dispatch.py \
       --manifest "$WA1_ROOT/manifests/track1_it2v.json" \
       --video-root "$VIDEO_ROOT" \
       --models pretrain round0 t4g_wmapA_pre_seed42_s250 \
       --output-dir "$WA1_ROOT/mlr_alg1" --python /path/to/python --num-shards 8 \
       --profile appendix_alg1_sam2_occlusion

   python benchmarks/worldarena/recompute_video_first_mlr.py \
     --input "$WA1_ROOT/mlr_alg1/summary.json" \
     --output "$WA1_ROOT/mlr_alg1/summary_video_first.json"
   ```

   `mlr_occlusion.py --list-profiles` prints every preset in `mlr_protocol_profiles.yaml`; `mlr_eval.py --compare-all` scores a single video under all of them at once (useful for checking how much a rule change moves the count before committing to a full run). Individual flags (`--deviation-mode`, `--occlusion-rule`, `--tau-occ`, `--sampled-count-source`, …) override whatever the profile sets.

   The presets split by what they hold fixed: `worldarena1_mlr_gdino_v2` is this repository's frozen WorldArena 1.0 rule (over-count only, gripper filter on); `raw_symmetric_count` is the symmetric count-only rule on raw detections; `appendix_alg1_sam2_occlusion` is the appendix's Algorithm 1 with the gripper-overlap filter switched off (the algorithm does not define one) and `appendix_alg1_sam2_occlusion_filtered` is the same rule with it left on, so the filter's contribution can be measured separately; `appendix_alg1_probe_detectors` keeps the detector knobs of the SAM2 probe; `sam2_presence_only` and `contact_recovery_gate` are the presence-only and contact-then-recovery exemption variants; `occlusion_from_raw_counts` drives the Algorithm 1 rule from raw `len(objects)` counts. `--sampled-count-source filtered|raw` (default `filtered`) picks the basis of the sampled count `N_t`; the same basis is used on the over-count and the under-count side, so both rules always read one count.
...
- **Dependencies.** The metric path needs the `WorldArena` conda environment (see [`docs/ENVIRONMENT.md`](../../docs/ENVIRONMENT.md)) plus the evaluator checkpoints verified in `prepare_evaluator.sh`. The MLR path needs GroundingDINO weights; set the model path with `GDINO_PATH` (read by `mlr_eval.py` / `mlr_occlusion.py`, or directly in [`eveworld/pipeline/t4g_gdino.py`](../../eveworld/pipeline/t4g_gdino.py)). The SAM2 occlusion rules additionally need `pip install git+https://github.com/facebookresearch/sam2` plus a checkpoint passed as `--sam2-checkpoint` or `SAM2_CHECKPOINT`; the counting path runs without SAM2 when `--occlusion-rule none`.
- **Detection stack location.** `mlr_eval.py` and `mlr_occlusion.py` import `t4g_detect`, `t4g_exam_v2`, and `t4g_gdino` from [`eveworld/pipeline/`](../../eveworld/pipeline/) (the import root is resolved relative to this file); `mlr_occlusion.py` builds its video predictor through the installed `sam2` package.
...
- **Protocol settings.** The parser and detector settings (transport-verb/object lists, box thresholds, gripper-overlap rule, 24 sampled frames, two-consecutive-timestamp rule) live in `mlr_protocol_profiles.yaml` as complete, self-consistent presets. `mlr_eval.py` and `mlr_dispatch.py` default to `worldarena1_mlr_gdino_v2` — the frozen protocol of the WorldArena 1.0 run in this repository — so previously produced shards stay comparable; `appendix_alg1_sam2_occlusion` (and its `_filtered` twin) is the occlusion-aware rule of the appendix. Pick a profile explicitly for new experiments and record the profile name next to any numbers you publish.
- **Count basis.** `--sampled-count-source filtered|raw` (default `filtered`) decides what the sampled count `N_t` is: spatially merged target detections minus gripper-overlap hits, or the raw `len(objects)` of GroundingDINO. The chosen basis is used for both the over-count and the under-count side of the comparison, so a single `N_t` drives both rules; the reference inventory `N_0` has its own `--initial-count-source`. Set both to `raw` for the plainest reading, both to `filtered` for the repository's detection hygiene.
