...
- **Cluster scripts.** All `kjob_*.sh` / `launch_*.sh` files are SLURM-style wrappers (`#SBATCH` headers, `kjobctl` submission, 8-GPU nodes) written for the authors' cluster. Absolute paths (`/data/datasets/gagi`, conda envs `giga_world1` / `EVEWorld` / `WorldArena`, `~/miniconda`) **must be adapted** to your environment before use. The generation payloads refuse to run on a GPU-less workspace host unless `ALLOW_LOCAL_RUN=1`; `xmodel_dreamgen_infer.py --dry-run` is the intended CPU-only sanity check.
...
- **Dependencies.** Generation needs the `giga_world1` conda env (diffusers ≥ 0.39 with all four I2V pipelines); judging uses the `EVEWorld` env; WorldArena core metrics use the separate WorldArena evaluator env (see [`../../docs/ENVIRONMENT.md`](../../docs/ENVIRONMENT.md)). Downloading uses the `hf` CLI and is CPU-only.
